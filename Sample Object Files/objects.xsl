...
>
	...
	<!-- UPDATES -->
		...
		-->
			<xsl:function name="ycba_fn:date-type">
				...
				<xsl:variable name="isDate" select="tokenize($date, '[0-9]{4}-(0[1-9]{1})|(1[0-1])-[0-3][0-9]')[1]=''"/>
		...
		>	
			...
			<!-- Acquisition -->
				<xsl:for-each select="//*[local-name() = 'descriptiveMetadata']/*[local-name() = 'eventWrap']/*[local-name() = 'eventSet']/*[local-name() = 'event'][*[local-name() = 'eventType']/*[local-name() = 'term']='acquisition']">
					...
					<xsl:element name="rdf:Description">
						...
						<xsl:if test="*[local-name() = 'eventMethod']/*[local-name() = 'term']">
						...
						<!--xsl:for-each select="../../../../*[local-name() = 'administrativeMetadata']/*[local-name() = 'rightsWorkWrap']/*[local-name() = 'rightsWorkSet']">
							<xsl:element name="rdfs:label">
								<xsl:value-of select="concat('Transferred to ', *[local-name() = 'creditLine'])"/>
							</xsl:element>
						</xsl:for-each -->
			...
			<!-- Object Rights -->
				<xsl:for-each select="//*[local-name() = 'descriptiveMetadata']/*[local-name() = 'rightsWorkWrap']/*[local-name() = 'rightsWorkSet']">
					<xsl:element name="rdf:Description">
						<xsl:attribute name="rdf:about" select="concat('&ycba;object/', ../../*[local-name() = 'recordWrap']/*[local-name() = 'recordID']/text(), '/objectRight')"/>
						<xsl:element name="rdf:type">
							<xsl:attribute name="rdf:resource" select="'&crm;E30_Right'"/>
						</xsl:element>
						<xsl:element name="crm:P3_has_note">
			...
			<!-- Exhibitions -->
				<xsl:for-each select="//*[local-name() = 'descriptiveMetadata']/*[local-name() = 'eventWrap']/*[local-name() = 'eventSet']/*[local-name() = 'event'][*[local-name() = 'eventType']/*[local-name() = 'term']='exhibition']">
					...
					<xsl:for-each select="*[local-name() = 'eventActor']/*[local-name() = 'actorInRole']/*[local-name() = 'actor']">
						...
						<xsl:variable name="roleActorPath" select="../*[local-name() = 'roleActor'][lower-case(*[local-name() = 'conceptID']/@*[local-name() = 'label'])='exhibition related constituent role']"/>
						<xsl:variable name="roleActor" select="$roleActorPath/*[local-name() = 'term']"/>
						<xsl:variable name="roleActorUri" >
							<xsl:choose>
								<xsl:when test="$roleActorPath/*[local-name() = 'conceptID']/@*[local-name() = 'source']='AAT'">
									<xsl:value-of select="concat('&getty_aat;', $roleActorPath/*[local-name() = 'conceptID'])"/>
								</xsl:when>
								<xsl:otherwise>
					...
					<xsl:element name="rdf:Description">
						...
						<xsl:for-each select="$objectMeasurements/*[local-name() = 'measurementsSet']">
							<xsl:element name="crm:P40_observed_dimension">
								<xsl:attribute name="rdf:resource"  select="concat($objectUri, '/', *[local-name() = 'measurementType'])"/>
							</xsl:element>
						</xsl:for-each>
    ...
    <xsl:template match="*[local-name() = 'eventActor']">
		<xsl:if test="*[local-name() = 'actorInRole']/*[local-name() = 'actor']/@*[local-name() = 'type']='person'">
			<xsl:variable name="aID" select="*[local-name() = 'actorInRole']/*[local-name() = 'actor']/*[local-name() = 'actorID']/text()"/>
			<xsl:variable name="actorID">
			   <xsl:choose>
